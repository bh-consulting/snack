...
function logsSearchFromDate(link) {
	var from_datetime = link.text().replace('s', '').split(' à ');
	var from_date = from_datetime[0].split('/');
	var from_time = from_datetime[1].split(/ et |h/);
	...
	if(to_datetime[1] != undefined) {
		...
		var unix_from = new Date(from_date[2], from_date[1], from_date[0], from_time[0], from_time[1], from_time[2]).getTime();
