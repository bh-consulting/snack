...
class LoglinesController extends AppController {
	...
	public function index() {
		...
		$this->Logline->set($this->request->data);

		if($this->Logline->validates()) {
			...
			// Search filter Severity
			{
				...
				foreach(array_reverse($this->Logline->severities) as $keyword => $severity) {
			...
			$this->set('filtersPanOpen', false);
		} else
			$this->set('filtersPanOpen', true);
		...
		$this->set('severities', $this->Logline->severities);
	}
